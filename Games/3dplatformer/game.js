...
const bloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),0.75,0.85,0.78);composer.addPass(bloom);

// Lighting
const hemi=new THREE.HemisphereLight(0x9fdfff,0x0a1724,2.0);scene.add(hemi);
const sun=new THREE.DirectionalLight(0xfff1d2,4.4);sun.position.set(-30,50,10);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-70;sun.shadow.camera.right=70;sun.shadow.camera.top=70;sun.shadow.camera.bottom=-70;scene.add(sun);
const fill=new THREE.DirectionalLight(0x58bfff,1.15);fill.position.set(30,10,-30);scene.add(fill);
...
const MAT={
  ...
  gold:new THREE.MeshStandardMaterial({color:0xffd768,emissive:0x7a4d00,emissiveIntensity:1.3,roughness:.24,metalness:.35}),
  portal:new THREE.MeshStandardMaterial({color:0x72ecff,emissive:0x1a7d9b,emissiveIntensity:5,roughness:.1,metalness:.2,transparent:true,opacity:.94}),
  enemy:new THREE.MeshStandardMaterial({color:0xf05d66,emissive:0x50141a,emissiveIntensity:1.5,roughness:.42}),
  ...
  playerAccent:new THREE.MeshStandardMaterial({color:0xffd46b,emissive:0x714700,emissiveIntensity:.85,roughness:.2,metalness:.25})
};
...
function addHazard(x,y,z,w=1.6,d=1.6){
  const m=addBox(world,MAT.enemy,[x,y+.12,z],[w,.18,d],{castShadow:false});m.material= new THREE.MeshStandardMaterial({color:0xff4058,emissive:0x7a0b1d,emissiveIntensity:1.4,roughness:.42});hazards.push({x,z,y:y+.2,w,d,mesh:m});
}
...
const visor=new THREE.Mesh(geo.box(.54,.2,.08),new THREE.MeshStandardMaterial({color:0x06131e,emissive:0x3ad7ff,emissiveIntensity:1.5,roughness:.2,metalness:.35}));visor.position.set(0,1.88,.47);player.add(visor);
...
function animate(now){const dt=Math.min(.033,(now-last)/1000);last=now;state.time+=dt;if(state.mode==='playing')state.runTime+=dt;
  updateWorld(dt);updateEffects(dt);if(state.mode==='playing')updatePlayer(dt);updateCamera(dt);composer.render();requestAnimationFrame(animate)}
