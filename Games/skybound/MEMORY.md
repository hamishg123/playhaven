...
The latest interaction pass adds a two-jump budget that resets on landing, smoother grounded braking and air steering, and a jump-edge queue so holding the key does not repeatedly consume jumps. Dev Studio now lets OrbitControls receive held left-drag gestures; selection is resolved on pointer-up only when the pointer stayed still, preventing camera orbit from being mistaken for a click. Explorer selection remains available as a reliable fallback.
